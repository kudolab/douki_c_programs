/* playrec_multi.c */
//
// Created by Tetsu Takizawa on 2019/11/20.
//
// clang playrec_multi.c -o playrec_multi -I /usr/local/include /usr/local/Cellar/portaudio/19.6.0/lib/libportaudio.a -framework CoreAudio -framework AudioToolbox -framework AudioUnit -framework CoreServices
...
#define CLEAR(a, ch) memset( (a), 0, FRAMES_PER_BUFFER * ch * SAMPLE_SIZE )
...
#define CLEAR(a, ch) { \
    int i; \
    for( i=0; i<FRAMES_PER_BUFFER*ch; i++ ) \
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 5) {
        ...
        fprintf(stderr, "douki_mono mcode_file(.DSB) douki_num channel_num output_filename(.DSB)\n");
        exit(EXIT_FAILURE);
    }
    mfilename = argv[1];
    ...
    AnyFileWrite(argv[4], &rec_raw_data[DOUKI_START * NUM_CHANNELS],
