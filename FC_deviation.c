...
int main(int argc, char *argv[])
{
	...
	char name1[256]  = "/ECTF/cECTF_";	// 読み込みファイル名(定数１)
...
// --------------------------------------------偏差の導出--------------------------------------------------------
	...
	// ----------ファイルに書き込み保存(テキスト出力:周波数-偏差特性概要)---------------------------------------------
		...
		strcat(name_out1,"_ECTF_standard_diviation");

		strcat(data_name,argv[1]);
		strcat(data_name,"/ECTF/cECTF_***_");
		if(completion == 0)
	...
	// ----------------ファイルに書き込み保存(グラフ出力:周波数-偏差特性)---------------------------------------------
		...
		if(completion == 0)
			strcat(name_out2,"_ECTF_div_character_L.txt");
		else
			strcat(name_out2,"_ECTF_div_character_R.txt");
